...
/* ------------ OpenSSL providers (legacy para DES) ------------ */
...
/* ------------ key(long) -> DES key (8 bytes, paridad impar) ------------ */
...
static void long_to_des8(uint64_t key, unsigned char out8[8]) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        unsigned char data = (unsigned char)(b & 0xFEu); /* limpia paridad */
        int ones = 0; for (int j = 1; j < 8; ++j) ones += (data >> j) & 1;
        unsigned char parity = (ones % 2 == 0) ? 1u : 0u; /* impar */
        out8[i] = (unsigned char)(data | parity);
    }
}

/* ------------ DES ECB sin padding (EVP) ------------ */
...
/* Substring a buscar (por defecto "D", configurable con -s) */
...
int main(int argc, char *argv[]){
    ...
    /* División por bloques [lower, upper) */
    ...
    /* Early-stop + métricas locales */
    uint64_t found = UINT64_MAX;
    uint64_t local_tests = 0;
    int status_code = 0; /* 0=done(range), 1=stop(signal), 2=found */
    int found_rank = -1;
    ...
    /* Cerrar la Irecv de forma segura */
