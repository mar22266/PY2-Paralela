...
static void banner(void){
    puts("============================================================");
    puts("  BruteDES • MPI+OpenMP (híbrido cíclico/strided)");
    puts("  - Llaves intercaladas: k = L + rank + t*P");
    puts("  - Early-stop no bloqueante");
    puts("  - OpenMP threads dentro de cada rank");
    puts("============================================================\n");
}
...
static void usage(const char *p){
    ...
    fprintf(stderr, "  Usa OMP_NUM_THREADS para controlar hilos por proceso\n");
}
...
int main(int argc,char**argv){
    ...
    // detectar numero de hilos OpenMP por proceso
    int num_threads = omp_get_max_threads();
    if(id==0){ 
        banner();
        printf("→ Configuración híbrida MPI+OpenMP\n");
        printf("  • Procesos MPI  : %d\n", P);
        printf("  • Hilos OpenMP/rank : %d\n", num_threads);
    ...
    // Variables compartidas entre hilos OpenMP
    int found_flag = 0;  // flag atómico para early-stop
    uint64_t found_key = UINT64_MAX;  // llave encontrada (compartida)
    
    // Paralelización híbrida: MPI (ranks) + OpenMP (threads dentro de cada rank)
    #pragma omp parallel shared(found_flag, found_key, local_tests, cipher, clen, needle_b, allow_stop, req, st)
    {
        int tid = omp_get_thread_num();
        int nthreads = omp_get_num_threads();
        uint64_t my_tests = 0;
        
        // Cada hilo dentro del rank procesa un subconjunto del rango del rank
        // Distribución: rank maneja k = L + id + t*P
        //               thread maneja cada nthreads-ésima llave del rango del rank
        for(uint64_t k = L + (uint64_t)id + (uint64_t)tid * (uint64_t)P; 
            k < U && !found_flag; 
            k += (uint64_t)(P * nthreads)) {
            
            // Check MPI early-stop solo en thread 0 (evitar race en MPI_Test)
            if(allow_stop && tid == 0) {
                int flag=0; 
            ...
            // Si otro hilo ya encontró, parar
            if(found_flag) break;
            ...
            if(des_try_key(k, cipher, clen, needle_b)){
                #pragma omp critical
                {
                    if(found_key == UINT64_MAX) {  // primer hilo en encontrar
                        found_key = k;
                        #pragma omp atomic write
                        found_flag = 1;
                        
                        // Notificar a otros ranks (solo un hilo lo hace)
                        if(allow_stop) {
                            for(int p=0; p<P; p++) {
                                MPI_Send(&found_key, 1, MPI_UINT64_T, p, 777, comm);
                            }
                        }
                    }
                }
        ...
        // Reducir tests locales de cada hilo
        #pragma omp atomic
        local_tests += my_tests;
    }
    
    // Actualizar variables globales después de región paralela
    if(found_key != UINT64_MAX) {
        found = found_key;
        status_code = 2;
        found_rank = id;
    } else if(allow_stop && found_flag) {
        status_code = 1;  // parado por señal externa
    }
    ...
    if(id==0){
        printf("→ BRUTEFORCE MPI+OpenMP (híbrido cíclico)\n");
        printf("  • Procesos   : %d\n", P);
        printf("  • Hilos/rank : %d\n", num_threads);
        printf("  • Total paralelo : %d\n", P * num_threads);
